# -*- coding: utf-8 -*-
"""
Created on Thu Jan 30 15:36:55 2025
...
def mask_rescaler(in1):
    ...
    return ypad
'''
def mask_rescaler(in1, scale_factor=0.3):
    """
    Resizes the input binary mask while ensuring it matches the fixed SLM dimensions by padding.
    
    Parameters:
    - in1: The input binary mask (numpy array).
    - scale_factor: A multiplier for resizing (e.g., 1.0 = original size, 0.5 = half size, 2.0 = double size)
    - If scale_factor=1.0, the function behaves exactly as before.
    - If scale_factor < 1.0, the image is scaled down and padded to keep the correct size.
    - If scale_factor > 1.0, the image is scaled up but cropped if it exceeds bounds.
    
    Returns:
    - Resized and padded mask as an 8-bit image.
    """
    # Compute new scaled dimensions
    new_h = int(h * scale_factor)
    new_w = int(w * scale_factor)

    # Resize the input mask
    y1 = resize(in1, (new_h, new_w), anti_aliasing=True)

    # Convert to uint8 binary format
    y1 = np.array(y1 * 255, dtype='uint8')

    # Create a blank image of the correct SLM size
    ypad = np.zeros((h, w), dtype='uint8')

    # Compute the offsets to center the scaled image
    start_y = (h - new_h) // 2
    start_x = (w - new_w) // 2

    # Insert the resized image into the center of the blank SLM-sized image
    ypad[start_y:start_y + new_h, start_x:start_x + new_w] = y1
